...
fn draw_sidebar(f: &mut Frame, app: &Arc<AppState>, ui: &Ui, area: Rect) {
    ...
    lines.push(gradient_line(" ╭─────────────────╮", ui.tick));
    lines.push(gradient_line(" │   DEBUG  PROXY   │", ui.tick.wrapping_add(3)));
    lines.push(gradient_line(" ╰─────────────────╯", ui.tick.wrapping_add(6)));
    lines.push(Line::default());
...
fn draw_log(f: &mut Frame, ui: &mut Ui, area: Rect) {
    ...
    let visible: Vec<Line> = ui.lines[ui.scroll..end].to_vec();
